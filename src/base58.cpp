...
namespace {
    class CEscrowAddressVisitor : public boost::static_visitor<bool> {
    private:
        CEscrowAddress *addr;
    public:
        CEscrowAddressVisitor(CEscrowAddress *addrIn) : addr(addrIn) { }
...
bool CEscrowAddress::Set(const CKeyID &id) {
    SetData(Params().Base58Prefix(CChainParams::PUBKEY_ADDRESS), &id, 20);
    return true;
}

bool CEscrowAddress::Set(const CScriptID &id) {
    SetData(Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS), &id, 20);
    return true;
}

bool CEscrowAddress::Set(const CTxDestination &dest) {
    return boost::apply_visitor(CEscrowAddressVisitor(this), dest);
}

bool CEscrowAddress::IsValid() const {
...
CTxDestination CEscrowAddress::Get() const {
    if (!IsValid())
...
bool CEscrowAddress::GetKeyID(CKeyID &keyID) const {
...
bool CEscrowAddress::IsScript() const {
    return IsValid() && vchVersion == Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS);
}

void CEscrowSecret::SetKey(const CKey& vchSecret) {
...
CKey CEscrowSecret::GetKey() {
    CKey ret;
    ret.Set(&vchData[0], &vchData[32], vchData.size() > 32 && vchData[32] == 1);
    return ret;
}

bool CEscrowSecret::IsValid() const {
    bool fExpectedFormat = vchData.size() == 32 || (vchData.size() == 33 && vchData[32] == 1);
    bool fCorrectVersion = vchVersion == Params().Base58Prefix(CChainParams::SECRET_KEY);
    return fExpectedFormat && fCorrectVersion;
}

bool CEscrowSecret::SetString(const char* pszSecret) {
    return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CEscrowSecret::SetString(const std::string& strSecret) {
