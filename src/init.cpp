...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Escrow.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: Escrowd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeEscrowamount=<n> " + _("Keep N Escrow anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Escrow is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Escrow is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Escrow version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Escrow server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Escrow") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Escrow to complete") << "\n";
    ...
    nAnonymizeEscrowAmount = GetArg("-anonymizeEscrowamount", 0);
    if(nAnonymizeEscrowAmount > 999999) nAnonymizeEscrowAmount = 999999;
    if(nAnonymizeEscrowAmount < 2) nAnonymizeEscrowAmount = 2;
    ...
    LogPrintf("Anonymize Escrow Amount %d\n", nAnonymizeEscrowAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1ESCO+1000 == (.1ESCO+100)*10
       10ESCO+10000 == (1ESCO+1000)*10
    */
