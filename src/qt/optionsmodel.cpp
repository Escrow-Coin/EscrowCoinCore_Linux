...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeEscrowAmount"))
        settings.setValue("nAnonymizeEscrowAmount", 1000);
    nAnonymizeEscrowAmount = settings.value("nAnonymizeEscrowAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeEscrowAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeEscrowAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeEscrowAmount:
            return QVariant(nAnonymizeEscrowAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeEscrowAmount:
            nAnonymizeEscrowAmount = value.toInt();
            settings.setValue("nAnonymizeEscrowAmount", nAnonymizeEscrowAmount);
            emit AnonymizeEscrowAmountChanged(nAnonymizeEscrowAmount);
            break;
        default:
            break;
        }
