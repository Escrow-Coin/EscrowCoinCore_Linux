...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeEscrowAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeEscrowAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeEscrowAmount = strAnonymizeEscrowAmount.remove(strAnonymizeEscrowAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeEscrowAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeEscrowAmount*COIN) nMaxToAnonymize = nAnonymizeEscrowAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeEscrowAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeEscrowAmount));
        strAnonymizeEscrowAmount = strAnonymizeEscrowAmount.remove(strAnonymizeEscrowAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeEscrowAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
