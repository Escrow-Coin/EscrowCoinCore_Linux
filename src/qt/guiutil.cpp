...
{
    // Convert Escrow:// to Escrow:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Escrow://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "Escrow:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Escrow.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Escrow.lnk
...
{
    return GetAutostartDir() / "Escrow.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a Escrow.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Escrow\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the Escrow app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add Escrow app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Escrow-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Escrow-qt [" + tr("command-line options") + "]                     " + "\n";
