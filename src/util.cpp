...
int nAnonymizeEscrowAmount = 1000;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Escrow
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Escrow
    // Mac: ~/Library/Application Support/Escrow
    // Unix: ~/.Escrow
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Escrow";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Escrow.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "Escrowd.pid"));
